...
import { fetchPairForTokenAddress } from "./swapFactory";
...
let PAIR_TOKEN_CONV_PAIR_INDEX: Address[] = [eth_address, uni_address];
...
let PAIR_TOKEN_PROPERTY: pairTokenProperty[] = [
  {
    Symbol: "CLP",
    Name: "ETH-USDT LP",
    Address: eth_usdt_pair_address, // Address
  ...
  },
  {
    Symbol: "CLP",
    Name: "UNI-USDT LP",
    Address: eth_uni_pair_address, // Address
    Decimals: BigInt.fromI32(18), // BigInt
    Token0: uni_address,
    Token1: eth_address,
  },
...
//获取token对于usd的价格,必须是该pair 没有usdt参与的情况
function getTokenUSDPrice(token: Token): BigDecimal {
  if (Address.fromString(token.id) == Address.fromString(erc20_usdt_address)) {
    return BigDecimal.fromString("1");
  }
  let convPair = Pair.load(conv_usdt_pair_address);
  let convPrice = ZERO_BD;

  if (Address.fromString(convPair.token0) == Address.fromString(erc20_usdt_address)) {
    convPrice = convPair.token0Price;
  } else {
    convPrice = convPair.token1Price;
  }

  if (Address.fromString(token.id) == Address.fromString(conv_address)) {
    return convPrice;
  }

  let pairId = getConvPairTokenAddress(Address.fromString(token.id));
  let pair = Pair.load(pairId.toHexString());
  if (Address.fromString(pair.token0) == Address.fromString(conv_address)) {
...
function getPairTokenPrice(pair: Pair): BigDecimal {
  ...
  if (Address.fromString(token0.id) == Address.fromString(erc20_usdt_address)) {
    token0USDprice = BigDecimal.fromString("1");
    token1USDprice = pair.token0Price;
  } else if (Address.fromString(token1.id) == Address.fromString(erc20_usdt_address)) {
...
function tokenUsdPriceUpdated(token0: Token, token1: Token, event: ethereum.Event): void {
